import KelasHeader from '@/components/pages/instansi/akademik/kelas/header'
import PageHeader from '@/components/shared/page-header'
import { Card, TabGroup } from '@/components/ui'
import { routes } from '@/config/routes'
import { metaObject } from '@/config/site.config'
import { lihatKelasAction } from '@/services/actions/instansi/akademik/kelas/lihat'
import {
  dehydrate,
  HydrationBoundary,
  QueryClient,
...
export default async function KelasLayout({
  ...
  await queryClient.setQueryData(
    ['instansi.akademik.kelas.lihat', idKelas],
    data
  )
    ...
    <>
      ...
      <HydrationBoundary state={dehydrate(queryClient)}>
        <Card className="flex flex-col">
          ...
          <TabGroup
            className="mt-2 mb-2"
            path={`${routes.instansi.kelasAkademik}/${idKelas}`}
            items={[
              {
