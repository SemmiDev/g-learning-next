...
import PengajarChartPresensiCard from './pengajar-chart-card'
import PengajarKehadiranPresensiCard from './pengajar-kehadiran-card'
import PengajarRekapPresensiCard from './pengajar-rekap-card'

const COLORS = ['#FFD1D1', '#D68585', '#B92E5D', '#6D1A36']

type PengajarPresensiBodyProps = {
  kelas?: DataKelasType
}

export default function PengajarPresensiBody({
  kelas,
}: PengajarPresensiBodyProps) {
  const chartData = [
    { name: 'Hadir', value: 60 },
    { name: 'Izin', value: 25 },
    { name: 'Sakit', value: 10 },
  ...
  return (
    <div className="grid grid-cols-3 gap-6 mt-8">
      <PengajarChartPresensiCard
        className="col-span-3 lg:col-span-1"
        data={chartData}
        colors={COLORS}
      />
      <PengajarKehadiranPresensiCard columns={tableColumns} data={tableData} />
      <PengajarRekapPresensiCard />
    </div>
  )
}
