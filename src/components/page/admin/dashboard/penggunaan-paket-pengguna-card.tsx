import { chartPenggunaanPaketPenggunaAction } from '@/actions/admin/dashboard/chart-penggunaan-paket-pengguna'
...
export default function DashboardPenggunaanPaketPenggunaCard({
  ...
  const { data = { list: [], max: 0 } } = useQuery<ChartType>({
    ...
    queryFn: async () => {
      const { data } = await chartPenggunaanPaketPenggunaAction()
