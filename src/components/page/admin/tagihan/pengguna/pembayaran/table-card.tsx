...
import { hapusPembayaranTagihanPenggunaAction } from '@/actions/admin/tagihan-pengguna/pembayaran/hapus'
...
export default function TablePembayaranTagihanPenggunaCard() {
  ...
  const queryKeyTagihan = ['admin.tagihan-pengguna.pembayaran', idTagihan]

  const { data: dataTagihan } = useQuery({
    queryKey: queryKeyTagihan,
    queryFn: makeSimpleQueryDataWithId(lihatTagihanPenggunaAction, idTagihan),
  })
  ...
  const {
    ...
    queryKey,
    action: tablePembayaranTagihanPenggunaAction,
  ...
  const handleHapus = () => {
    ...
    handleActionWithToast(
      hapusPembayaranTagihanPenggunaAction(idTagihan, idHapus),
      {
        loading: 'Menghapus...',
        onSuccess: () => {
  ...
  return (
    <>
      <Card className="p-0">
        <div className="flex flex-col justify-between gap-x-6 gap-y-2 px-2.5 py-3 sm:flex-row sm:items-center">
          <div className="flex flex-wrap gap-x-6 gap-y-2">
            ...
            />
            <RincianItem label="Pengguna" value={dataTagihan?.nama_pengguna} />
      ...
      <ModalConfirm
        title="Hapus Pembayaran Tagihan Pengguna"
