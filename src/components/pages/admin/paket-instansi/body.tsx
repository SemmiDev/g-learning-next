...
import { hapusPaketInstansiApi } from '@/services/api/admin/paket-instansi/hapus'
import { listPaketInstansiApi } from '@/services/api/admin/paket-instansi/list'
  ...
  return (
    <>
      <div className="flex flex-col gap-y-4">
        ...
        {list.length ? (
          <>
            <div className="grid grid-cols-1 items-start gap-5 mt-4 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 3xl:grid-cols-5">
              {list.map((item) => (
                <PaketItemCard
                  key={item.id}
                  paket={item}
