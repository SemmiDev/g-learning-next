...
import { hapusTagihanPenggunaApi } from '@/services/api/admin/tagihan-pengguna/hapus'
import { tableTagihanPenggunaApi } from '@/services/api/admin/tagihan-pengguna/table'
...
const queryKey = ['admin.tagihan-pengguna.table'] as const

export default function TableTagihanPenggunaCard() {
  const {
    show: showUbah,
    key: keyUbah,
    doShow: doShowUbah,
  ...
  const {
    ...
    action: hapusTagihanPenggunaApi,
    refetchKey: queryKey,
  })
  ...
  const {
    ...
    queryKey,
    action: tableTagihanPenggunaApi,
  })
  ...
  const tableColumns: ColumnsType<(typeof data)[number]> = [
    {
      title: (
        <TableHeaderCell
          title="Nama Pengguna"
          sortable
          sort={getSortOrder(sort, 'nama_pengguna')}
        />
      ),
    ...
    },
    {
      ...
      render: (_, row) => (
        <div className="flex justify-center">
          <Link href={`${routes.admin.tagihanPengguna}/${row.id}`}>
            <ActionIconTooltip
              tooltip="Pembayaran"
              size="sm"
              as="span"
